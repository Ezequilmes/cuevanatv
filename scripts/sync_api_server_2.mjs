...
app.post("/api/sync-movie", async (req, res) => {
  ...
  try {
    ...
    // Insertamos directamente la data web cruda (como en tu versión de respaldo)
        ...
        const scanAndProcess = async (dir) => {
            ...
            for (const item of items) {
                ...
                if (stat.isDirectory()) {
                    ...
                    // LIMPIEZA INALTERABLE DEL NOMBRE PARA EL source_page_url
                    const stableBaseName = baseName.replace(/(Dual|Latino|1080p|720p|4k|WEB-DL|HDRip|x264|x265)/gi, '').trim();

                    // Normalización de URL para el archivo físico
                    const publicUrl = cleanUrl(fullPath);

                    // 2. IDENTIDAD INMUTABLE: Definir source_page_url basándose estrictamente en el archivo
                    const stableSourceUrl = isSeries
                        ? `series://${encodeURIComponent(stableBaseName.toLowerCase())}`
                        : publicUrl;

                    // 3. PROTECCIÓN CONTRA DUPLICADOS (SSOT)
                    if (isSeries) {
                    ...
                    const intelligence = await fetchSeriesMetadata(item);
...
async function fetchSeriesMetadata(nombreArchivo) {
    // 1. INTENTO PRIMARIO: TMDB (Datos Oficiales)
    const cleanName = nombreArchivo.replace(/\.(mp4|mkv|avi|mov)$/i, '').replace(/\./g, ' ').replace(/[sS](\d{1,2})[eE](\d{2}).*/i, '').trim();
    const isSeriesGuess = /[sS](\d{1,2})[eE](\d{2})/i.test(nombreArchivo);

    // Extraer temporada y episodio del nombre del archivo si es serie
    const match = nombreArchivo.match(/[sS](\d{1,2})[eE](\d{2})/i);
    const season = match ? parseInt(match[1]) : 1;
    const episode = match ? parseInt(match[2]) : 1;

    try {
        const tmdbData = await fetchTMDBMetadata(cleanName, isSeriesGuess ? 'series' : 'movie');
        if (tmdbData) {
            console.log(`🎬 TMDB encontró metadatos para: ${cleanName}`);
            return {
                type: isSeriesGuess ? 'series' : 'movie',
                title: tmdbData.title,
                season: season,
                episode: episode,
                description: tmdbData.description || "",
                category: isSeriesGuess ? "Series" : "Novedades",
                poster_url: tmdbData.poster_url
            };
        }
    } catch (e) {
        console.warn(`⚠️ TMDB falló en el escaneo automático para ${cleanName}, usando fallback local...`);
    }

    // 2. FALLBACK DETERMINISTA (Cero IA)
    console.log(`🛡️ Usando fallback protegido para: ${cleanName}`);
    return {
        type: isSeriesGuess ? 'series' : 'movie',
        title: cleanName, // Forzamos el nombre limpio del archivo
        season: season,
        episode: episode,
        description: "Sincronizado desde almacenamiento local.",
        category: isSeriesGuess ? "Series" : "Novedades",
        poster_url: `https://ui-avatars.com/api/?name=${encodeURIComponent(cleanName)}&background=000&color=fff&size=512`
    };
}
...
app.post("/api/admin/tmdb-bulk-refresh", async (req, res) => {
    try {
        ...
        const { data: titles } = await supabase.from('titles').select('id, title, type').neq('type', 'live').is('poster_url', '');
